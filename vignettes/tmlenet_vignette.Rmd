...
<!-- ================================================ -->
 ...
 \begin{description}
 \item [\emph{{def.sW}}] This is the first part of the two part specification of the structural equation model
    for the outcome \texttt{Y}.
 ...
 \item [\emph{{def.sA}}] Defines treatment summary measures \texttt{sA} that can be functions of each unit's exposure and baseline covariates, 
    ...
    as well as the exposure \texttt{Anode}.
 \item [\emph{{eval.summaries}}] A convenience function that can be used for validating and evaluating the user-specified summary measures. 
 ...
 \item [\emph{{tmlenet}}] Performs estimation of the causal effect of interest using the observed input \texttt{data}, 
...
constructing the dataset of $N$ summary measures, ${(A^s_i,W^s_i : i=1,...,N)}$, and then fitting the usual i.i.d. MLE 
...
Observed data summary measures (sW,sA) and the network are stored in one object named \texttt{DatNet.ObsP0}:
...
```{r}
names(res1$EY_gstar1$h_g0_SummariesModel)
names(res1$EY_gstar1$h_gstar_SummariesModel)
```
