#-----------------------------------------------------------------------------
# ALGORITHM
# New summary measure routine, allows for arbitrary network summary measures of covariates and treatments
# Allows the output to not depend on K, simplifying estimation functions fit_hbar and pred_hbar...
#-----------------------------------------------------------------------------
# NOTES.
# *) Need to come up with a consistent and simple naming scheme for friends' Vars: rename netW1_j to W1_netj?
# *) How to define and pass the summary measure functon? W_i^s will be a list of R expressions, captured with substitute()... (like nodes)
...
# (II) DISCRETIZE ALL CONTINUOUS SUMMARY MEASUREs in sA (do not touch Ws)
# A: create NBins dummy indicators
      ...
      # Could define indicators as cdf or prob mass function, no difference (see approach 1 & 2)!
...
testsubst <- function(data, subsetexpr, nodes) {
# ... copied 
}
datatest <- data.frame(
...
a0 <- testsubst(subsetexpr = TRUE, data = datatest, nodes = nodes)
nF0 <- testsubst(subsetexpr = (nFnode == 0), data = datatest, nodes = nodes)
all.equal(nF0, datatest$nFriends==0)
nF1 <- testsubst(subsetexpr = (nFnode == 1), data = datatest, nodes = nodes)
all.equal(nF1, datatest$nFriends==1)
nFandA <- testsubst(subsetexpr = (nFnode == 0 | Anode == 0), data = datatest, nodes = nodes)
all.equal(nFandA, datatest$nFriends == 0 | datatest$A == 0)
