...
def.nodeojb <- function(datO) {
  ...
  netind_cl <- tmlenet:::NetIndClass$new(nobs = nrow(datO))
  # Define datNetObs:
  datnetW <- tmlenet:::DatNet$new(netind_cl = netind_cl, nodes = nodes, VarNodes = nodes$Wnodes, addnFnode = TRUE)$make.sVar(Odata = datO, sVar.object = def_sW)
  datnetA <- tmlenet:::DatNet$new(netind_cl = netind_cl, nodes = nodes, VarNodes = nodes$Anode)$make.sVar(Odata = datO, sVar.object = def_sA)
  datNetObs <- tmlenet:::DatNet.sWsA$new(datnetW = datnetW, datnetA = datnetA)$make.dat.sWsA()
...
test.simple.fit.density.sA <- function() {
  ...
  regclass <- tmlenet:::RegressionClass$new(outvar.class = sA_class, outvar = reg.sVars$outvars, predvars = reg.sVars$predvars, subset = subsets_expr)
  summeas.g0 <- tmlenet:::SummariesModel$new(reg = regclass, O.datnetA = nodeobjs$datNetObs$datnetA)
