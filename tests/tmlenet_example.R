...
# W2[,0] or W2[[0]] just means W2
# W2[ , 1:Kmax] or W2[[1:Kmax]] means vectors of (W2_netF_j), j=1, ..., Kmax
# sum(W2[ , 1:Kmax]) or W2[[1:Kmax]] is netW2_sum
...
Qform <- "Y ~ I("%+%paste(str_c(netvar2("W2", (1:6)), "*", "(1-",netvar2("A", (1:6)), ")"), collapse = "+")%+%")"%+%"+"%+%"I("%+%paste(netvar2("W3", (1:6)), collapse = "+")%+%")"
# [1] "Y ~ I(W2_netF1*(1-A_netF1)+W2_netF2*(1-A_netF2)+W2_netF3*(1-A_netF3)+W2_netF4*(1-A_netF4)+W2_netF5*(1-A_netF5)+W2_netF6*(1-A_netF6))+I(W3_netF1+W3_netF2+W3_netF3+W3_netF4+W3_netF5+W3_netF6)"
gform <- "A ~  W1 + netW1_sum + netW2_sum + netW3_sum + nFriends"
hform <- "sA ~ "%+%paste(netvar2("W2", (1:6)), collapse = "+")%+%" + netW3_sum + nFriends"
# [1] "sA ~ W2_netF1+W2_netF2+W2_netF3+W2_netF4+W2_netF5+W2_netF6 + netW3_sum + nFriends"
#----------------------------------------------------------------------------------
...
tmlenet_K6out2 <- tmlenet(data=df_K6, Anode='A', Wnodes=Wnodes, Ynode='Y', nFnode='nFriends',
						k_max=kmax, IDnode='IDs', NETIDnode='Net_str', Qform=Qform, gform=gform, h_form=hform,
						f.g1.star=f.A_0, f.g1_args=NULL, n_MCsims=10, n_samp_g0gstar=10, alphaTMLE_B_only = FALSE)
)
# N=1,000
...
tmlenet_K6out1$estimates$EY_g1.star$tmle_B
tmlenet_K6out1$estimates$EY_g1.star$CI_tmle_B_iidIC
...
tmlenet_out1 <- tmlenet(data=sample_network_k2, Anode='A', Wnodes='W1', Ynode='Y', nFnode='nFriends',
						k_max=2, IDnode='IDs', NETIDnode='Net_str', Qform=Qform, gform=gform, 
