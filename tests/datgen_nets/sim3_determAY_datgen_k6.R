...
.f.Qform <- function(k, qform_Vars=NULL, k_miss=1, miss=F) {
	...
	if (k > 0) { 
		...
		qform_NetVarsW3 <- paste("netW3_", c(1: k), sep = "")
...
.f.gform <- function(k, k_miss=1, gform_Vars, miss=F) {
	...
	if (miss) k <- k_miss
	if (k > 0) {	
		gform_NetVars1 <- paste("netW1_", c(1: k), sep = "")
		gform_NetVars2 <- paste("netW2_", c(1: k), sep = "")
		gform_NetVars3 <- paste("netW3_", c(1: k), sep = "")
...
f.A <- function(k, data, ...) {
	...
	netW3 <- subset(data, select = netW3_1:eval(parse(text=paste("netW3_", k, sep = ""))))
...
gendata_Cj <- function(C_j = 1, n, k, EC, f.g_name=NULL, f.g_args=NULL) { 
			...
			if (nFriendSamp>0) {
				...
				# W1-based probs of i being selected
					friends_i <- sort(sample(FriendSampSet, nFriendSamp, 
										prob=prob_f[W1[FriendSampSet]+1]))
				}		
				I[friends_i, index] <- 1
  ...
  #-----------------------------------------------    	
	...
	# Generate asymmetric connectivity matrix of friends
 	...
 	#Update # of actual friends sampled, |N_i|
  	...
  	W1_netW <- data.frame(.f.redefineCov(W1, "W1"))
   	...
   	W3_netW <- data.frame(.f.redefineCov(W3, "W3"))
   	W_netW <- cbind(W1_netW, W2_netW, W3_netW)
	# print("W_netW"); print(head(W_netW))
	...
	# SUM netW for each W separately
 	...
 	# print("netW123_sum"); print(head(netW123_sum))
